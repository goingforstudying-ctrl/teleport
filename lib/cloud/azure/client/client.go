...
func FetchResourceGraphPage[T any](ctx context.Context, query, skipToken string, subscriptions []string, client *Client, tokenProvider azcore.TokenCredential) (*T, error) {
	...
	return DoRequest[](ctx, client, request)
}
...
func DoRequest[T any](ctx context.Context, client *Client, request *http.Request) (*T, error) {
	...
	for {
		...
		if err != nil {
			rateLimitErr, isRetryable := isRateLimitError(err)
			if isRetryable && client.retryOnRateLimitErrors  && currentRetry <= maxRetriesAfterRateLimitErrors {
...
func isRateLimitError(err error) (bool, *azure.RateLimitError) {
	if rateLimitErr, isRateLimitErr := errors.AsType[*azure.RateLimitError](err); isRateLimitErr {
