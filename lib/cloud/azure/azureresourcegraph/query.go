...
import (
	...
	"github.com/gravitational/teleport/lib/cloud/azure/httpclient"
)
...
func CollectAll[T any](ctx context.Context, logger *slog.Logger, client *httpclient.Client, tokenProvider azcore.TokenCredential, typedQuery TypedQuery[T], queryOpts ...QueryOption) ([]T, error) {
	var results []T
	for item, err := range Iterator(ctx, logger, client, tokenProvider, typedQuery, queryOpts...) {
		if err != nil {
...
func Iterator[T any](ctx context.Context, logger *slog.Logger, client *httpclient.Client, tokenProvider azcore.TokenCredential, typedQuery TypedQuery[T], queryOpts ...QueryOption) iter.Seq2[T, error] {
	...
	return func(yield func(T, error) bool) {
		...
		for subscriptionsChunk := range slices.Chunk(client.Subscriptions(), 1000) {
			for currentPage := 1; currentPage <= resourceGraphMaxPages; currentPage++ {
				page, err := fetchPage(ctx, queryString, skipToken, subscriptionsChunk, client, tokenProvider)
				if err != nil {
					maxPageRetries--
...
type resourcesAPIResponse struct {
	...
	Error httpclient.ResourcesAPIError `json:"error"`
}
