...
func TestValidateApp(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "scoped derived public_addr accepted",
			app:  scopedApp("grafana", scopedScope, scopedDerived),
		},
		{
			name: "scoped public_addr with different proxy suffix accepted (label-only)",
			app:  scopedApp("grafana", scopedScope, scopedapp.ScopedAppPublicAddr(scopedScope, "grafana", "other.example.com")),
		},
		{
			name:    "scoped empty public_addr rejected",
			app:     scopedApp("grafana", scopedScope, ""),
			wantErr: "does not match its derived address",
		},
		{
			name:    "scoped wrong scope's derived address rejected",
			app:     scopedApp("grafana", scopedScope, scopedapp.ScopedAppPublicAddr("/prod", "grafana", "proxy.example.com")),
			wantErr: "does not match its derived address",
		},
		{
