...
var postgresImages []string = []string{
	"postgres:14",
	"postgres:15",
	"postgres:16",
	"postgres:17",
...
//
// setupSQL runs through env.bootstrapConn (superuser) because the
...
func runReassignSuccess(t *testing.T, env *testEnv, username, setupSQL string) {
	t.Helper()
	require.NoError(t, env.engine.ActivateUser(t.Context(), makeReassignSession(env.db, username, nil)))
	_, err := env.bootstrapConn.Exec(t.Context(), setupSQL)
	require.NoError(t, err)
	require.NoError(t, env.engine.DeleteUser(t.Context(), makeReassignSession(env.db, username, nil)))
	require.False(t, userExists(t, env.adminConn, username), "user should be dropped after reassignment")
}
...
// runReassignSuccess for why setup runs through env.bootstrapConn.
func runReassignRefused(t *testing.T, env *testEnv, username, setupSQL string) {
	t.Helper()
	require.NoError(t, env.engine.ActivateUser(t.Context(), makeReassignSession(env.db, username, nil)))
	_, err := env.bootstrapConn.Exec(t.Context(), setupSQL)
	require.NoError(t, err)
...
// container, the database resource, the engine, and the four pgx
// connections used throughout (two as superuser for bootstrap-only
// operations, two as teleport-admin for everything under test). All
// connection cleanup is registered on t.Cleanup, so callers receive a
// ready-to-use env and never need to close anything explicitly.
type testEnv struct {
	db                   types.Database
	engine               *Engine
	bootstrapConn        *pgx.Conn // postgres superuser, postgres db
	bootstrapOtherDBConn *pgx.Conn // postgres superuser, other_db
	adminConn            *pgx.Conn // teleport-admin, postgres db
	otherDBConn          *pgx.Conn // teleport-admin, other_db
	port                 string    // mapped host port (e.g. "32768")
	connStr              string    // superuser connection URL for the postgres db
}

// setupTestEnv spins up the postgres testcontainer, provisions the
...
// up via t.Cleanup.
func setupTestEnv(t *testing.T, postgresImage string) *testEnv {
	...
	bootstrapOtherDBConn, err := pgx.Connect(t.Context(),
		fmt.Sprintf("postgres://postgres:postgres@localhost:%s/other_db", port))
	require.NoError(t, err)
	...
	adminConn, err := pgx.Connect(t.Context(),
		fmt.Sprintf("postgres://teleport-admin:admin@localhost:%s/postgres", port))
	require.NoError(t, err)
	...
	otherDBConn, err := pgx.Connect(t.Context(),
		fmt.Sprintf("postgres://teleport-admin:admin@localhost:%s/other_db", port))
	require.NoError(t, err)
...
func TestActivateUser(t *testing.T) {
	for _, postgresImage := range postgresImages {
		t.Run(postgresImage, func(t *testing.T) {
			env := setupTestEnv(t, postgresImage)
			...
			t.Run("active connection same roles succeeds", func(t *testing.T) {
				...
				userConn, err := pgx.Connect(t.Context(),
					fmt.Sprintf("postgres://alice_active_same:testpass@localhost:%s/postgres", port))
				require.NoError(t, err)
			...
			t.Run("active connection different roles fails", func(t *testing.T) {
				...
				userConn, err := pgx.Connect(t.Context(),
					fmt.Sprintf("postgres://alice_active_diff:testpass@localhost:%s/postgres", port))
				require.NoError(t, err)
				defer userConn.Close(context.Background())
	...
	for _, postgresImage := range postgresImages {
		t.Run(postgresImage, func(t *testing.T) {
			...
			t.Run("is no-op when user has active connections", func(t *testing.T) {
				...
				userConn, err := pgx.Connect(t.Context(),
					fmt.Sprintf("postgres://alice_deact_active:testpass@localhost:%s/postgres", port))
				require.NoError(t, err)
				defer userConn.Close(context.Background())
	...
	for _, postgresImage := range postgresImages {
		t.Run(postgresImage, func(t *testing.T) {
			...
			t.Run("does not drop user with active connections", func(t *testing.T) {
				...
				userConn, err := pgx.Connect(t.Context(),
					fmt.Sprintf("postgres://alice_delete_active:testpass@localhost:%s/postgres", port))
				require.NoError(t, err)
				defer userConn.Close(context.Background())
...
func TestDeleteUserWithReassignment(t *testing.T) {
	for _, postgresImage := range postgresImages {
		t.Run(postgresImage, func(t *testing.T) {
			env := setupTestEnv(t, postgresImage)
			...
			port, connStr := env.port, env.connStr
			...
			t.Run("Reassign: table with index", func(t *testing.T) {
				runReassignSuccess(t, env, "alice_re_idx", `
			...
			t.Run("Reassign: table with identity sequence", func(t *testing.T) {
				...
				runReassignSuccess(t, env, "alice_re_idseq", `
			...
			t.Run("Reassign: composite row type follows", func(t *testing.T) {
				...
				runReassignSuccess(t, env, "alice_re_rowtype", `
			...
			t.Run("Reassign: array type follows", func(t *testing.T) {
				...
				runReassignSuccess(t, env, "alice_re_arr", `
			...
			t.Run("Reassign: FK constraint internal triggers are ignored", func(t *testing.T) {
				...
				runReassignSuccess(t, env, "alice_re_fk", `
			...
			t.Run("Reassign: standalone sequence", func(t *testing.T) {
				runReassignSuccess(t, env, "alice_re_seq", `
			...
			t.Run("Reassign: custom schema with contents", func(t *testing.T) {
				runReassignSuccess(t, env, "alice_re_schtbl", `
			...
			t.Run("Reassign: mixed safe types", func(t *testing.T) {
				runReassignSuccess(t, env, "alice_re_mix", `
			...
			t.Run("Refused: partition child", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_pchild", `
			...
			t.Run("Refused: partitioned parent", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_pparent", `
			...
			t.Run("Refused: table with row-level security", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_rls", `
			...
			t.Run("Refused: table with SECURITY INVOKER trigger", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_tinv", `
			...
			t.Run("Refused: table with SECURITY DEFINER trigger", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_tdef", `
			...
			t.Run("Refused: function owned by user", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_fn", `
			...
			t.Run("Refused: view owned by user", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_view", `
			...
			t.Run("Refused: materialized view owned by user", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_mv", `
			...
			t.Run("Refused: foreign table owned by user", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_ft", `
			...
			t.Run("Refused: standalone composite type", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_ct", `
			...
			t.Run("Refused: domain owned by user", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_dom", `
			...
			t.Run("Refused: enum type owned by user", func(t *testing.T) {
				runReassignRefused(t, env, "alice_no_enum", `
			...
			t.Run("Reassign: login attempt during reassignment is rejected", func(t *testing.T) {
				...
				lockConn, err := pgx.Connect(t.Context(), connStr)
				require.NoError(t, err)
				...
				_, loginErr := pgx.Connect(t.Context(),
					fmt.Sprintf("postgres://alice_login_race:testpass@localhost:%s/postgres", port))
			...
			t.Run("Reassign: current-db active session bails before reassign", func(t *testing.T) {
				...
				userConn, err := pgx.Connect(t.Context(),
					fmt.Sprintf("postgres://alice_re_active:testpass@localhost:%s/postgres", port))
				require.NoError(t, err)
				defer userConn.Close(context.Background())
			...
			t.Run("Reassign: other-db active session restores LOGIN", func(t *testing.T) {
				...
				otherSessionConn, err := pgx.Connect(t.Context(),
					fmt.Sprintf("postgres://alice_re_otherdb:testpass@localhost:%s/other_db", port))
				require.NoError(t, err)
