...
func validateAccessListMemberOrOwnerNesting(
	ctx context.Context,
	parentList *accesslist.AccessList,
	memberOrOwnerName accesslist.ScopeQualifiedName,
...
func validateMemberOrOwnerScopeHierarchy(
	...
	if list.GetScope() == "" {
		return trace.BadParameter("Access List '%s' cannot name '%s' as %s because scoped access lists cannot be members or owners of unscoped access lists",
			list.Spec.Title, ScopeQualifiedNameToString(memberOrOwnerName), kindStr)
	}

	if !scopes.PolicyResourceScope(list.GetScope()).CanDependOnStateFromPolicyResourceAtScope(memberOrOwnerName.Scope) {
		return trace.BadParameter("Access List '%s' cannot name '%s' as %s because it is at a descendent scope",
			list.Spec.Title, ScopeQualifiedNameToString(memberOrOwnerName), kindStr)
	}
...
func validateAddition(
	...
	// Cycle detection
	reachable, err := isReachable(ctx, childList, parentList, make(map[accesslist.ScopeQualifiedName]struct{}), g)
	if err != nil {
		return trace.Wrap(err)
	}
	if reachable {
...
func isReachable(
	...
	visited map[accesslist.ScopeQualifiedName]struct{},
	g AccessListAndMembersGetter,
) (bool, error) {
	if currentList.GetScopeQualifiedName() == targetList.GetScopeQualifiedName() {
		return true, nil
	}
	if _, ok := visited[currentList.GetScopeQualifiedName()]; ok {
		return false, nil
	}
	visited[currentList.GetScopeQualifiedName()] = struct{}{}

	// Traverse member lists
	listMembers, err := fetchMembers(ctx, currentList.GetScopeQualifiedName(), g)
	if err != nil {
...
func exceedsMaxDepth(
	...
	case RelationshipKindOwner:
		...
		depthDownwards, err := maxDepthDownwards(ctx, childList.GetScopeQualifiedName(), make(map[accesslist.ScopeQualifiedName]struct{}), g)
		if err != nil {
			return false, trace.Wrap(err)
		}
	...
	default:
		...
		depthUpwards, err := maxDepthUpwards(ctx, parentList, make(map[accesslist.ScopeQualifiedName]struct{}), g)
		if err != nil {
			return false, trace.Wrap(err)
		}
		depthDownwards, err := maxDepthDownwards(ctx, childList.GetScopeQualifiedName(), make(map[accesslist.ScopeQualifiedName]struct{}), g)
