...
endpoint (RFD 318), so you query and filter rather than reconstruct paths.
...
| `--from`     | Show activity at/after this time; **enables** the activity columns. RFC3339, `YYYY-MM-DD`, `24h`, `7d`. |
...
| `--detailed` | Expand each access into one row per grantor, with each grantor's individual level.       |
