...
```
Identity   Kind  Resource    Resource Kind  Access Level  Granted By     Grantor Counts  [Accesses  Last Access]
```

**Detailed (`--detailed`)** — replaces `Granted By`/`Grantor Counts` with one row per
grantor (each grantor's own level). A resource with multiple grantors gets a
