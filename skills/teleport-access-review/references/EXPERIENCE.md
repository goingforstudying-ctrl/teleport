...
$TCTL access-review --query "SELECT * FROM access_path WHERE resource ILIKE '%prod-db%'" \
  --limit 50 --format json | jq -r '[.identities[].resources[]?.resource.name] | unique[]'
...
$TCTL access-review --from 90d --query "…" --format json \
  | jq -r '.identities[] | .identity.name as $i
           | .resources[] | [$i, .resource.name, .level,
...
$TCTL access-review --query "SELECT * FROM access_path WHERE resource ILIKE 'prod-db%'" --format json \
  | jq -r '.identities[] | select(.resources|length>0)
           | [.identity.name, .resources[0].level, .resources[0].grantors[0].node.name] | @tsv'
```
...
$TCTL access-review --from 90d --query "SELECT * FROM access_path WHERE resource ILIKE 'prod-db%'" --format json \
  | jq -r '.identities[] | .identity.name as $i | .resources[]
...
jq -r '.identities[].resources[]
       ...
       | [.resource.name, .resource.sub_kind, (.grantors[0].node.name // "?")] | @tsv' /tmp/alice.json
# Used standing access — keep:
jq -r '.identities[].resources[] | select(.level=="standing" and (.activity.count // 0)>0)
       | [.resource.name, .activity.count, .activity.last_access] | @tsv' /tmp/alice.json
```
...
- **Activity requires a window.** No `--from` → no activity columns at all.
  `--to` without `--from` is rejected; `--from` must be before `--to` (a future
  `--from` is rejected).
